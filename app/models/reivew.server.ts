...
const generateReviewViaPrompt = async (
  ...
  const response = await ai.models.generateContent({
    ...
    config: {
      systemInstruction: `You are a helpful assistant that generates product reviews. The reviews must be in JSON format and adhere to the following structure: { "rating": number, "author": string,"title" :string, "imageUrl": string, "productId": string, "testimonial": string, "like":int }. The title should be a maximum of 5 words.The description should be maximum 40 words. The imageUrl can be an empty string. The rating is between 1-5 but out of the amount you generate, generate a mixture of numbers but more 4 and 5. For the productId field use this productId:${id}. For like make it a decent numbe somewhere between 10 - 100. For companyResponse make it a meaningful response based off of the testimonial you have generated. With the testimonal, make it more human like, so some gramattical errors etc.. `,
